...
// GetUserHireInfo godoc
// @Summary Get user public hire info
// @Description Get public hire info by username
// @Tags User
// @Accept json
// @Produce json
// @Security ApiKeyAuth
// @Param username query string true "username"
// @Success 200 {object} handler.RespBody{data=schema.PublicHireInfoResp}
// @Router /answer/api/v1/user/hire [get]
func (uc *UserController) GetUserHireInfo(ctx *gin.Context) {
	req := &schema.GetUserHireInfoReq{}
	if handler.BindAndCheck(ctx, req) {
		return
	}
	resp, err := uc.userService.GetPublicHireInfoByUsername(ctx, req.Username)
	handler.HandleResponse(ctx, err, resp)
}

// UpdateUserHireInfo godoc
// @Summary Update current user's hire info
// @Description Update current user's hire info
// @Tags User
// @Accept json
// @Produce json
// @Security ApiKeyAuth
// @Param data body schema.UpdateHireInfoReq true "UpdateHireInfoReq"
// @Success 200 {object} handler.RespBody{}
// @Router /answer/api/v1/user/hire [put]
func (uc *UserController) UpdateUserHireInfo(ctx *gin.Context) {
	req := &schema.UpdateHireInfoReq{}
	if handler.BindAndCheck(ctx, req) {
		return
	}
	req.UserID = middleware.GetLoginUserIDFromContext(ctx)
	errFields, err := uc.userService.UpdateHireInfo(ctx, req)
