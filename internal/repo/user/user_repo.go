...
// UpdateHireInfo updates hire-related fields for a user
func (ur *userRepo) UpdateHireInfo(ctx context.Context, userID string, hire *entity.User) (err error) {
	_, err = ur.data.DB.Context(ctx).Where("id = ?", userID).
		Cols("hire_me_enabled", "hire_rate_currency", "hire_rate_amount", "hire_rate_unit",
			"hire_contact_email", "hire_contact_url", "hire_note", "hire_last_updated_at").
		Update(hire)
	if err != nil {
		err = errors.InternalServer(reason.DatabaseError).WithError(err).WithStack()
	}
