...
// UpdateHireInfo updates current user's hire info
func (us *UserService) UpdateHireInfo(ctx context.Context, req *schema.UpdateHireInfoReq) (errFields []*validator.FormErrorField, err error) {
	// basic validation already done by BindAndCheck
	oldUserInfo, exist, err := us.userRepo.GetByUserID(ctx, req.UserID)
	if err != nil {
		return nil, err
	}
	...
	// map req to entity fields
	update := &entity.User{
		ID:                oldUserInfo.ID,
		Bio:               oldUserInfo.Bio,
		BioHTML:           oldUserInfo.BioHTML,
		HireNote:          req.Note,
		HireMeEnabled:     req.Enabled,
		HireLastUpdatedAt: time.Now(),
	}
	if req.Rate != nil {
		update.HireRateCurrency = req.Rate.Currency
		update.HireRateAmount = int(req.Rate.Amount)
		update.HireRateUnit = req.Rate.Unit
	} else {
		update.HireRateCurrency = ""
		update.HireRateAmount = 0
		update.HireRateUnit = ""
	}
	if req.Contact != nil {
		update.HireContactEmail = req.Contact.Email
		update.HireContactURL = req.Contact.URL
	} else {
		update.HireContactEmail = ""
		update.HireContactURL = ""
	}

	err = us.userRepo.UpdateHireInfo(ctx, req.UserID, update)
	if err != nil {
		return nil, err
	}
	return nil, nil
...
// GetPublicHireInfoByUsername returns public hire info if enabled and feature flag allows
func (us *UserService) GetPublicHireInfoByUsername(ctx context.Context, username string) (*schema.PublicHireInfoResp, error) {
	...
	if !userInfo.HireMeEnabled {
		return nil, errors.NotFound(reason.ObjectNotFound)
	}
	resp := &schema.PublicHireInfoResp{Enabled: true}
	if userInfo.HireRateCurrency != "" || userInfo.HireRateAmount > 0 || userInfo.HireRateUnit != "" {
		resp.Rate = &schema.HireRate{
			Currency: userInfo.HireRateCurrency,
			Amount:   int64(userInfo.HireRateAmount),
			Unit:     userInfo.HireRateUnit,
		}
	}
	if userInfo.HireContactEmail != "" || userInfo.HireContactURL != "" {
		resp.Contact = &schema.HireContact{
			Email: userInfo.HireContactEmail,
			URL:   userInfo.HireContactURL,
		}
	}
	resp.Note = userInfo.HireNote
	return resp, nil
}
