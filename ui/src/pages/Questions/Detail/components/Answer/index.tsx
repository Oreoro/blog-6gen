...
import { useHireInfoByUsername, acceptanceAnswer } from '@/services';
...
const Index: FC<Props> = ({
  ...
  const { data: hireInfo } = useHireInfoByUsername(username);
  ...
  return (
    <div id={data.id} ref={answerRef} className="answer-item py-4">
      ...
      <div className="d-flex align-items-center mt-4">
        ...
        {hireInfo?.enabled && (
          <HireMeButton className="ms-3" onClick={() => setShowHire(true)} />
        )}
      </div>
      <HireMeModal
        show={showHire}
        onHide={() => setShowHire(false)}
        data={hireInfo}
      />
