...
import type { UcSettingAgent } from '@/services';
...
const Index: React.FC = () => {
  ...
  const [hireAmount, setHireAmount] = useState<number | ''>('');
  const [hireUnit, setHireUnit] = useState<'hour' | 'day' | 'project'>('hour');
  ...
  const handleHireSave = async (e: FormEvent) => {
    e.preventDefault();
    try {
      await updateMyHireInfo({
        enabled: hireEnabled,
        rate:
          hireAmount !== ''
            ? {
                currency: hireCurrency,
                amount: Number(hireAmount),
                unit: hireUnit,
              }
            : undefined,
        contact: { email: hireEmail || undefined, url: hireURL || undefined },
        note: hireNote,
      });
      toast.onShow({ msg: t('hire.saved'), variant: 'success' });
    } catch (err) {
  ...
  return (
    <>
      ...
      {/* Hire Me settings */}
      {!ucAgent?.enabled || profileAgent?.enabled === false ? (
        <Form className="mt-5" onSubmit={handleHireSave}>
          <h4 className="mb-3">{t('hire.hire_me')}</h4>
          <Form.Check
            type="switch"
            id="hire-enabled"
            label={t('hire.hire_me')}
            checked={hireEnabled}
            onChange={(e) => setHireEnabled(e.currentTarget.checked)}
            className="mb-3"
          />
          <div className="d-flex flex-wrap gap-2 mb-3">
            <Form.Control
              value={hireCurrency}
              onChange={(e) => setHireCurrency(e.currentTarget.value)}
              placeholder="USD"
              className="w-auto"
            />
            <Form.Control
            ...
            <Form.Select
              value={hireUnit}
              onChange={(e) => setHireUnit(e.currentTarget.value as any)}
              className="w-auto">
              <option value="hour">{t('hire.per_hour')}</option>
              <option value="day">{t('hire.per_day')}</option>
              <option value="project">{t('hire.per_project')}</option>
            </Form.Select>
          </div>
          <div className="d-flex flex-wrap gap-2 mb-3">
            <Form.Control
              type="email"
              value={hireEmail}
              onChange={(e) => setHireEmail(e.currentTarget.value)}
              placeholder="email@example.com"
              className="w-auto"
            />
            <Form.Control
              value={hireURL}
              onChange={(e) => setHireURL(e.currentTarget.value)}
          ...
          <Form.Control
            ...
            rows={3}
            value={hireNote}
            onChange={(e) => setHireNote(e.currentTarget.value)}
            placeholder={t('hire.note') || ''}
            className="mb-3"
          />
          <Button type="submit" variant="primary">
            {t('btn_save', { keyPrefix: 'btns' })}
          </Button>
        </Form>
      ) : null}
    </>
