...
export const useHireInfoByUsername = (username: string) => {
  const apiUrl = '/answer/api/v1/user/hire';
  const { data, error } = useSWR<Type.PublicHireInfoResp | undefined, Error>(
    username ? `${apiUrl}?username=${username}` : null,
...
export const updateMyHireInfo = (payload: Type.UpdateHireInfoReq) => {
  return request.put('/answer/api/v1/user/hire', payload);
};
